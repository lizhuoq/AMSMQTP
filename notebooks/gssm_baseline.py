...
if __name__ == "__main__":
    ...
    parser.add_argument("--gssm_tibetan_dataset_root", default="../data/baseline/GSSM_processed/Tibetan", type=str)
    parser.add_argument("--gssm_cra_dataset_root", default="../data/baseline/GSSM_processed/CRA", type=str)
    ...
    gssm_tibetan_dataset_root = args.gssm_tibetan_dataset_root
    gssm_cra_dataset_root = args.gssm_cra_dataset_root
    iid = args.iid
    layer = args.layer
    ...
    root_setting = f"GSSM_split_method_{split_method}_layer_{layer}_iid_{iid}"
    ...
    # gssm filenames
    tibetan_filenames = [os.path.join(gssm_tibetan_dataset_root, layer, os.path.split(x)[-1]) for x in tibetan_filenames]
    cra_filenames = [os.path.join(gssm_cra_dataset_root, layer, os.path.split(x)[-1]) for x in cra_filenames]
    ...
    if split_method == 'spatial':
        ...
        # gssm
        train_ls = [pd.read_csv(x) for x in train_filenames if os.path.exists(x)]
        test_ls = [pd.read_csv(x) for x in test_filenames if os.path.exists(x)]
    else:
        df_ls = [pd.read_csv(x) for x in filenames if os.path.exists(x)]
    ...
    # add date_time
    idx = test_data[['station_idx', "GSSM", "soil_moisture", "date_time"]].copy()
    idx["date_time"] = pd.to_datetime(idx["date_time"])
    
    idx.rename(columns={"soil_moisture": "true", "GSSM": "pred"}, inplace=True)
    os.makedirs(test_results_root, exist_ok=True)
    idx.to_csv(os.path.join(test_results_root, 'test_results.csv'), index=False)

    # dropna
    idx.dropna(axis=0, how="any", inplace=True)

    # drop date_time
    idx.drop("date_time", axis=1, inplace=True)
