...
# discussion
def final_train(X_train, y_train, X_test, y_test, time_budget, estimator):
    ...
    automl_settings = {
        ...
        "eval_method": "cv", 
        "n_splits": 10, 
        "estimator_list": [estimator] if estimator != "auto" else estimator
    }
    automl.fit(
...
if __name__ == "__main__":
    ...
    # discussion
    parser.add_argument('--test_time_budget', action="store_true")
    parser.add_argument("--estimator", type=str, 
                        choices=['lgbm', 'xgboost', 'xgb_limitdepth', 'catboost', 'rf', 'extra_tree', "auto"])

    args = parser.parse_args()
    tibetan_dataset_root = args.tibetan_dataset_root
    cra_dataset_root = args.cra_dataset_root
    # discussion
    time_budget = args.time_budget if not args.test_time_budget else 2
    iid = args.iid
    layer = args.layer
    ...
    root_setting = f"{estimator}_split_method_{split_method}_layer_{layer}_iid_{iid}_time_budget_{time_budget}"
    ...
    if args.test_time_budget:
        ...
        # discussion
        automl = final_train(train_features, train_labels, test_features, test_labels, time_budget, estimator)
