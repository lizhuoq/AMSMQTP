...
if __name__ == "__main__":
    ...
    # load model
    # discussion
    estimators = ["catboost", "extra_tree", "lgbm", "rf", "xgb_limitdepth", "xgboost"]
    automl_lst = []
    feature_names_lst = []
    for estimator in estimators:
        with open(join(f"../discussion/checkpoints/{estimator}_split_method_spatial_layer_{LAYER}_iid_adversial_validation_time_budget_2400", "automl_total.pkl"), "rb") as f:
            automl = pickle.load(f)

        # load feature names
        with open(join(f"../discussion/checkpoints/{estimator}_split_method_spatial_layer_{LAYER}_iid_adversial_validation_time_budget_2400", "feature_names.pkl"), "rb") as f:
            feature_names = pickle.load(f)

        automl_lst.append(automl)
        feature_names_lst.append(feature_names)

        if len(feature_names_lst) > 1:
            assert feature_names_lst[-1] == feature_names_lst[-2]

    feature_names = feature_names_lst[0]
    ...
    for year in years:
        ...
        for month in months:
            ...
            for time in tqdm(times):
                ...
                # discussion
                for estimator, model in zip(estimators, automl_lst):
                    pred = model.predict(inference_df_withnonan)

                    inference_df_withnonan[estimator] = pred

                inference_df_withnonan["pred"] = inference_df_withnonan[estimators].mean(axis=1)
