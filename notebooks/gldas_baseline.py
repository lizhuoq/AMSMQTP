...
if __name__ == "__main__":
    ...
    root_setting = f"GLDASSM_split_method_{split_method}_layer_{layer}_iid_{iid}"
    ...
    gldas_col = layers_gldas[layer]

    # add date_time
    idx = test_data[['station_idx', gldas_col, "soil_moisture", "date_time"]].copy()
    idx["date_time"] = pd.to_datetime(idx["date_time"])

    idx.rename(columns={"soil_moisture": "true", gldas_col: "pred"}, inplace=True)
