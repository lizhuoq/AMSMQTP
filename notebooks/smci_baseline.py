...
if __name__ == "__main__":
    ...
    parser.add_argument("--smci_tibetan_dataset_root", default="../data/baseline/processed/Tibetan", type=str)
    parser.add_argument("--smci_cra_dataset_root", default="../data/baseline/processed/CRA", type=str)
    ...
    smci_tibetan_dataset_root = args.smci_tibetan_dataset_root
    smci_cra_dataset_root = args.smci_cra_dataset_root
    iid = args.iid
    layer = args.layer
    ...
    root_setting = f"SMCI_split_method_{split_method}_layer_{layer}_iid_{iid}"
    ...
    # smci filenames
    tibetan_filenames = [os.path.join(smci_tibetan_dataset_root, layer, os.path.split(x)[-1]) for x in tibetan_filenames]
    cra_filenames = [os.path.join(smci_cra_dataset_root, layer, os.path.split(x)[-1]) for x in cra_filenames]
    ...
    # add date_time
    idx = test_data[['station_idx', "SMCI", "soil_moisture", "date_time"]].copy()
    idx["date_time"] = pd.to_datetime(idx["date_time"])
    
    idx.rename(columns={"soil_moisture": "true", "SMCI": "pred"}, inplace=True)
