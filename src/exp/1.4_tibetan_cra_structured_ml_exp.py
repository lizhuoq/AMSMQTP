...
if __name__ == '__main__':
    ...
    if layer != 'layer4':
        tibetan_seed = tibetan_seed[split_method][layer]
    else:
        pass
    ...
    if split_method == 'spatial':
        if layer == 'layer4':
            ...
            # split_tibetan_filenames ############################################################################
            tibetan_train_filenames, tibetan_test_filenames = train_test_split(tibetan_filenames, test_size=0.2, shuffle=True, random_state=tibetan_seed)
            ############################################################################
            if layer != 'layer1':
                train_filenames, test_filenames = train_test_split(cra_filenames, test_size=0.2, shuffle=True, random_state=SEED)
                train_filenames += tibetan_train_filenames
                test_filenames += tibetan_test_filenames
            else:
                train_filenames = tibetan_train_filenames
                test_filenames = tibetan_test_filenames
    ...
    if split_method == 'temporal':
        filenames = cra_filenames + tibetan_filenames
        if opt.use_era5_mean_std:
    ...
    if split_method in ['spatial', 'temporal']:
        ...
        ###############################################################################
        df_train_adv = train_features.copy()
        df_test_adv = test_features.copy()
    ...
    if opt.del_features == 'Original':
        log_path = os.path.join(opt.log_root, f'tibetan_{tibetan_dataset_root.split("/")[-1]}_cra_{cra_dataset_root.split("/")[-1]}' + '_original_seed4cra.csv')
    else:
