...
parser = argparse.ArgumentParser(description='Train CRA Soil Moisture Structured dataset using AutoML')
...
if __name__ == '__main__':
    ...
    if split_method in ['spatial', 'temporal']:
        ...
        val_labels = val_data.soil_moisture
    ...
    if opt.del_features == 'Original':
        log_path = os.path.join(opt.log_root, f'cra_{cra_dataset_root.split("/")[-1]}' + '_original.csv')
    else:
